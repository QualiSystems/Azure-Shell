...
class DeployAzureVMOperation(object):
    ...
    def deploy(self, azure_vm_deployment_model, cloud_provider_model, reservation_id):
        ...
        resource_name = self._generate_name(azure_vm_deployment_model.app_name)
        ...
        network_name = self._generate_name(random_name)
        subnet_name = random_name
        ip_name = random_name
        storage_account_name = self._generate_name(random_name)
        ...
        # 4. create Vm
        vm_result = self.vm_service.create_vm(image_offer, image_publisher, image_sku, image_version, admin_password,
                                              admin_username,
                                              computer_name, group_name, nic_id, region, storage_account_name, vm_name)

        vm_result_json = jsonpickle.decode(vm_result._response.text)
